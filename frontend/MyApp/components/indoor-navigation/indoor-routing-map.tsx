...
export function IndoorRoutingMap({ destination, onRouteComputed }: Props) {
  ...
  useEffect(() => {
    ...
    const startId = endNode.floor === 3 ? "3N1" : "N1";
  ...
  useEffect(() => {
    ...
    const startNode = ALL_NODES.find((node) => node.id === selectedStartId);
    const endNode = ALL_NODES.find((node) => node.id === selectedEndId);
    if (!startNode || !endNode || startNode.floor !== endNode.floor) return;

    const floorNodes = ALL_NODES.filter((node) => node.floor === startNode.floor);
    ...
    const ids = dijkstra(floorNodes, floorEdges, selectedStartId, selectedEndId);
    const coords = ids
    ...
    setFloorPaths((prev) => ({ ...prev, [startNode.floor]: coords }));
    traversePath(coords, startNode.floor);
    if (onRouteComputed) onRouteComputed(ids);
  }, [onRouteComputed, selectedEndId, selectedStartId]);
  ...
  const runDijkstra = (startId?: string, endId?: string) => {
    ...
    const startNode = ALL_NODES.find((node) => node.id === start);
  ...
  return (
    <View style={styles.wrapper}>
        ...
        <View style={styles.selectorColumn}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {nodesOnCurrentFloor.map((node) => (
              <TouchableOpacity
                key={node.id}
                onPress={() => setSelectedStartId(node.id)}
                style={[
                  styles.nodeButton,
                  selectedStartId === node.id && styles.nodeButtonSelected,
                ]}>
                <Text
                  style={
                    selectedStartId === node.id
      ...
      <View style={styles.floorButtons}>
        ...
        <Button title="3rd Floor" onPress={() => runDijkstra("3N1", "3N4")} />
        <Button title="4th Floor" onPress={() => runDijkstra("N1", "N11")} />
      </View>
